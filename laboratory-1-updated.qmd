...
This experiment uses a Bouyoucos hydrometer to measure the density (grams per L) of a liquid mixture (“slurry”) of soil and water to determine the particle size distribution (sand, silt and clay percentages) of the fine earth portion of a soil sample. Using the hydrometer allows us to determine the grams of particular sizes of soil particles (sand, silt, and, clay) that remain suspended in the cylinder after a specific period of time. Different sized soil particles are separated by their different sedimentation rates – e.g. larger particles will settle faster in a column of water, while smaller particles remain suspended much longer in the solution (based on Stokes Law).

### Preparation

The cylinders in this investigation each contain 60 grams of oven-dry dispersed soil After mixing thor- oughly with a stir stick, the largest particles (sand) will quickly drop to the bottom of the cylinder. After 40 seconds, only silt and clay particles are left suspended in the water. After two hours, only clay-sized particles remain.
...
hydrometer_test_video <- qrcode::qr_code("
tinyurl.com/36p9bxcs")
...
1.  Carefully use the stirring rod (approximately 18 inches long with a disk on the end) to completely disperse the soil in the cylinder. This requires that you **quickly** lower and lift the stirring rod up and down in the cylinder until **all the sediment is removed from the bottom of the cylinder.**

2.  After stirring, immediately note the time to the nearest second. *Carefully* and *slowly* insert the hydrometer (**the hydrometers are extremely fragile**) into the cylinder. Please refer to the figure on the next page. *Note*: you may need to use your finger to stop the hydrometer from bobbing.

3.  After 40 seconds, read the number (at liquid level) on the hydrometer.

4.  This reading must be corrected for temperature. Add 0.4 g/L for each degree above 20º Celsius or subtract 0.4 g/L for each degree below 20º Celsius.
...
![The soil in the diagram above would have: (60-30)/60 x 100= 50% Sand; 10/60 x100= 17% Clay; 100-50-17=33% Silt)](hydrometer-Picture1.png){#fig-hydrometer}

**Calculations**: correct readings before calculating sand, silt, and clay!

$$
Sand\ (\%) = \frac{M_{sample,oven\ dry} - corrected\ 40s \ reading}{M_{sample,oven\ dry}}\ *\ 100\ 
$$

$$
Clay\ (\%) = \frac{corrected\ 2hr\ reading}{M_{sample,oven\ dry}}\ *\ 100\ 
$$
...
## BUILD TABLE
################
Measurements <- c("Sample mass (oven-dry)", "40s reading (uncorrected)", "Temperature @ 40s (C)", "40s reading (corrected)", "% Sand", "2hr reading (uncorrected)", "Temperature @ 2hr (C)", "2hr reading (corrected)", "% Clay", "% Silt")
Sample1 <- c("60 g", "g/L", "ºC", "g/L", "%", "g/L", "ºC", "g/L", "%", "%")
Sample2 <- c("60 g", "g/L", "ºC", "g/L", "%", "g/L", "ºC", "g/L", "%", "%")
...
::: callout-note
## Watch this video before you start Investigation C

```{r, echo = FALSE, message = FALSE, warning = FALSE, out.width = "25%"}
#note - control plotting size with fig.width and fig.height in chunk option above - numbers are in inches. see here: https://bookdown.org/yihui/rmarkdown-cookbook/figure-size.html - out.width works!!!!!
library(qrcode)

hydrometer_test_video <- qrcode::qr_code("https://tinyurl.com/yw9nm4tj")
#print(hydrometer_test_video)
plot(hydrometer_test_video)
```
:::
...
### Practicing Texture by Feel

Use the **known** texture samples to practice texturing soil with the "feel" method.

::: callout-warning
RINSE YOUR HANDS IN THE DUNK BUCKETS PROVIDED BETWEEN EVERY SAMPLE! DRY YOUR HANDS WITH A PROVIDED TOWEL. ASK THE TA IF THERE ARE NO CLEAN TOWELS.
:::
...
**Note**: In the table below, check one box to choose between the characteristics (gritty, smooth, or gritty/smooth) that you feel for each texture class.
...
### Estimating Unknown Textures by Feel
...
colnames(texturegrittysmooth) = c("Sample", "Dominated by gritty feel (Y/N)", "Dominated by smooth feel (Y/N)", "Equally gritty/smooth (Y/N)", "Ribbon length", "Textural Class Name")
...
Complete the charts below using lecture notes and the structural examples provided.
...
structurehorizon <- data.frame(Structure, Horizon, Characteristics)
colnames(structurehorizon) = c("Structure", "Common Master Horizon", "Characteristics")
...
tab1.1 = structurehorizon_ht %>% 
      ...
      set_valign(row = 1:row.num, col = 1:col.num, value = 'middle') %>%
      #set_number_format(2:3, 1, value = NA) %>% #; don't format these cells as numbers
      set_col_width(c(0.2, 0.2, 0.3, 0.3)) %>%
      set_latex_float(value = "h!") %>%
      #set_row_height(1:3, c(0.02, 0.2, 0.2)) %>%
      set_bottom_padding(row = 2:row.num, col= 1:col.num, 12) %>%
      set_top_padding(row = 2:row.num, col= 1:col.num, 12) %>%
