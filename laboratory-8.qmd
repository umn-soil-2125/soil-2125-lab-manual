...
1.  Match each soil profile with its location in Minnesota by the depth to CaCO~3~ (Bk or Ck). Use HCl to determine depths to carbonates.

![\"](depth-to-carbonates.png){#fig-carbonates} ![\"](mn-precip-evap.png){#fig-precipevap}

2.  For a more detailed and complex representation of the depth to carbonates in soils across Minnesota, see the depth to carbonates map in Soil Explorer on the iPads. In addition to climate, which two of the other four soil forming factors contribute the most to these patterns, and why?
...
tab1.1 = soilhistoricveg_ht %>% 
      ...
      set_latex_float(value = "h!") %>%
      #set_row_height(1:3, c(0.02, 0.2, 0.2)) %>%
      set_bottom_padding(row = 2:7, col= 1:2, 40) %>%
      set_top_padding(row = 2:7, col = 1:2, 40) %>%
      #set_row_height(c(0.02, 0.2, 0.2)) %>%
      set_all_borders(brdr(1, "solid", "black"))
      #set_caption("Pots of jam for sale") # this is if we want a caption - I don't think we need any captions but leaving it here just in case
