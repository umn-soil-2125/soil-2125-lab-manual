...
Observe each of the three tubes provided of a soil slurry (from a soil with a clay loam texture) that has been shaken with three different solutions: de-ionized water (containing virtually no ions), NaCl (containing dissolved Na+ cations), and AlCl3 (containing dissolved Al3+ cations). These tubes were shaken up and allowed to settle for approximately 8 hours.

Record your observations of each tube in the table below. PAY ATTENTION TO THE CLARITY OF THE LIQUID AT THE TOP OF EACH TUBE. High clarity indicates the absence of suspended fine clay particles, while cloudy liquid indicates that clay particles are still floating around in suspension and have not settled out.

**Record your observations of water clarity, depth to the settled soil particles from the top of the water column, and how easily you can see the “floccules” or conglomeration of clay particles caused by any introduced ions.**
...
slurryion <- data.frame(Slurry, Ion, WaterClarity, DepthtoSediment, Floccules)

colnames(slurryion) = c("Solution", "Ion", "Water Clarity", "Depth to Sediment", "Easy to See Floccules?")
...
tab1.1 = slurryion_ht %>% 
      ...
      #set_bottom_border(row = 1, col = everywhere) %>% # probably dont need this because you set all borders below
      set_width(0.9) %>% #; 1 == full width
      set_align(row = 1:4, col = 1:5, value = 'center') %>%
      #set_valign(row = 2:3, col = 1:2, value = 'middle') %>%
      #set_number_format(2:3, 1, value = NA) %>% #; don't format these cells as numbers
      #set_col_width(c(0.3, 0.6)) %>%
      set_latex_float(value = "h!") %>%
      #set_row_height(1:3, c(0.02, 0.2, 0.2)) %>%
      set_bottom_padding(4, 1:5, 10) %>%
...
## INVESTIGATION F: Soil Texture, Mineralogy, and Cracking Behavior

Two soils have been set out for you to look at. One is dominated by Kaolinite clays and one by Smectite clays. Both soils have a clay percentage greater than 35%. You should notice significant differences in cracking behavior when dried, which is related to both the
texture (how much clay is in the soil) AND the mineralogy. Unlike the Kaolinite and Smectite clays in Investigation E, which are from mined geological deposits, these are real soil materials (dominated by each of these different clay types) and so they have color associated with them which is not due to the clay minerals alone.
...
dominantmoist <- data.frame(DominantClayMineralinSoil, MoistCharacteristics, DryCharacteristics, GeneralLocationsofSoilinUS)
colnames(dominantmoist) = c("Dominant Clay Mineral in Soil", "Moist Characteristics", "Dry Characteristics", "General Locations of Soil in US")
...
## SET AS HUXTABLE TYPE
################
dominantmoist_ht <- as_hux(dominantmoist)
...
tab1.1 = dominantmoist_ht %>% 
      ...
      set_latex_float(value = "h!") %>%
      #set_row_height(1:3, c(0.02, 0.2, 0.2)) %>%
      set_bottom_padding(row = 2:3, col= 1:4, 60) %>%
      set_top_padding(row = 2:3, col = 1:4, 60) %>%
      #set_row_height(c(0.02, 0.2, 0.2)) %>%
      set_all_borders(brdr(1, "solid", "black"))
      #set_caption("Pots of jam for sale") # this is if we want a caption - I don't think we need any captions but leaving it here just in case
