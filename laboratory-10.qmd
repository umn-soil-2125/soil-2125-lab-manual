...
Use the properties and interpretations from each of the map units given in Web Soil Survey to fill in the following table. The information below the heading of each column shows you where to find that piece of information in Web Soil Survey menus:
...
Suborder <- c(">Suitability & Limitations for Use >Land Classification >Soil Taxonomy Classification", "", "", "", "", "", "", "", "")
SmallCommercialBuildings <- c(">Suitability & Limitations for Use >Building Site Development >Small Commercial Building", "", "", "", "", "", "", "", "")
PathsandTrails <- c(">Suitability & Limitations for Use >Recreational Development >Paths and Trails", "", "", "", "", "", "", "", "")
AtgradeSeptic <- c(">Suitability and Limitations for Use >Sanitary Facilities >Septic Tank Absorption Fields >At-Grade (MN)", "", "", "", "", "", "", "", "")
FarmlandofStatewideImportance <- c(">Suitability and Limitations for Use >Land Classifications >Farmland Classifications", "", "", "", "", "", "", "", "")
...
tab1.1 = symbolsoilorder_ht %>% 
      ...
      set_latex_float(value = "h!") %>%
      #set_row_height(1:3, c(0.02, 0.2, 0.2)) %>%
      set_bottom_padding(row = 2:10, col= 1:7, 20) %>%
      set_top_padding(row = 2:10, col = 1:7, 20) %>%
      #set_row_height(c(0.02, 0.2, 0.2)) %>%
      set_all_borders(brdr(1, "solid", "black"))
      #set_caption("Pots of jam for sale") # this is if we want a caption - I don't think we need any captions but leaving it here just in case
...
## BUILD TABLE
################
Facility <- c("PLant, Animal, and Environmental Science Building (small commercial)", "Septic system for whole unit (At-Grade Septic)", "Restored wetland (Aquic Suborders and/or Histosols", "Permanent soil pit displayed Farmland of Statewide Importance", "Nature and bike trail (Paths and Trails)")
Mostsuitable <- c("", "", "", "", "")
